...
import { CMix, DMClient, XXDKUtils } from "xxdk-wasm/dist/src";
...
export function XXDirectMessages({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const cipher = xx.NewDatabaseCipher(xxNet.GetID(), Buffer.from("MessageStoragePassword"), 725);
    ...
    const onDmEvent = (eventType: number, data: Uint8Array) => {
      ...
      if (db !== null) {
        ...
        Promise.all([
          db.table<DBDirectMessage>("messages").where("id").equals(e.uuid).first(),
          db
    ...
    xxdk.dmIndexedDbWorkerPath().then((workerPath: string) => {
      ...
      xx.NewDMClientWithIndexedDb(xxNet.GetID(), notifications.GetID(), cipher.GetID(), workerStr, dmID, {
  ...
  return (
    <XXDMClient.Provider value={dmClient}>
      <XXDMReceiver.Provider value={dmReceiver}>{children}</XXDMReceiver.Provider>
    </XXDMClient.Provider>
  );
}
  ...
  return (
    <>
      <div className="flex flex-grow p-2">
        <Input type="text" placeholder="Type message to send..." value={msgToSend} onChange={handleInputChange} />
      </div>
